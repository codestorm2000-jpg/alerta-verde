...
export default function RegisterModal({ isOpen, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      >
        <h2 className="text-2xl font-bold text-[#f8fafc] mb-6 text-center">Crear cuenta</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-[#94a3b8] mb-1.5">Confirmar contrasena</label>
            <input
              type="password"
              placeholder="••••••••"
              className="w-full bg-[#0f172a] border border-[#334155] text-[#f8fafc] rounded-lg px-4 py-3 text-sm focus:outline-none focus:border-[#32D04F] transition-colors placeholder:text-[#94a3b8]/50"
